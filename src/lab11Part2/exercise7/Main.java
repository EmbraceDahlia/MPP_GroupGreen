...
public class Main {
	...
	public static void ordering1(List<Integer> list) {
		System.out.println(list.stream()
				.sorted((x, y) -> {
					int absCompare = Integer.compare(Math.abs(x), Math.abs(y));
					if (absCompare != 0) {
						return absCompare; // sort by absolute value
					} else {
						return Integer.compare(x, y); // negatives before positives if same abs
					}
				})
				.toList());
	}
	...
	public static void ordering2(List<String> words) {
		System.out.println(words.stream().map(String::toLowerCase).map(s -> new StringBuilder(s).reverse().toString()).sorted().toList());
